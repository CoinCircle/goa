...
func NewExpandedStoredBottleCollection(res storage.ExpandedStoredBottleCollection) ExpandedStoredBottleCollection {
...
// NewExpandedStoredBottle builds the HTTP response body from the result of the
// "show" endpoint of the "storage" service.
func NewExpandedStoredBottle(res *storage.ExpandedStoredBottle) *ExpandedStoredBottle {
	body := &ExpandedStoredBottle{
		ID:          res.ID,
		Name:        res.Name,
		Vintage:     res.Vintage,
	...
	if res.Winery != nil {
		body.Winery = marshalExpandedWineryToExpandedWinery(res.Winery)
	}
	if res.Composition != nil {
		body.Composition = make([]*ExpandedComponent, len(res.Composition))
		for j, val := range res.Composition {
			body.Composition[j] = &ExpandedComponent{
				Varietal:   val.Varietal,
				Percentage: val.Percentage,
			}
		}
